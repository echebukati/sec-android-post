<?php
include_once 'psl-config_mobile.php';
...
function login($username, $password, $mysqli) {
  ...
  if ($stmt = $mysqli->prepare("SELECT id, password, salt FROM users WHERE username = ? LIMIT 1")) {
    $stmt->bind_param('s', $username);  // Bind "$username" and "$company" to parameter.
    ...
    if ($stmt->num_rows == 1) {
      ...
      if (checkbrute($username, $mysqli) == true) {
        ...
        if ($db_password == $password) {
          ...
          $mysqli->query("UPDATE users SET authenticationTime = NOW(), IP = '".$_SERVER['REMOTE_ADDR']."' WHERE id = ".$user_id." LIMIT 1");
          // Login successful.
          return true;
        } else {
